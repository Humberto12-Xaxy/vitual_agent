...
class VoiceAssistent:
        ...
        while self.active:
            ...
            with self.microphone as source:
                try:
                    ...
                    print(f'Dijiste: {self.text.lower()}')
                    ...
                    print(elapsed_time)
                    if self.text.lower() in phrases and self.is_speaking:
                        self.stop_audio()
                    
                    if self.text == 'reanuda' and not self.is_speaking:
                        self.resume_audio()
                    
                    if self.text and self.response == '' and not self.is_speaking and elapsed_time > 3:

                        print(self.text, self.response, self.is_speaking)
                        self.synthesize_speech('Estoy analizando la información, espere un momento')
                        self.play_audio()
                        sleep(3)
    ...
    def synthesize_speech_interrupt(self, text):
        ...
        if 'AudioStream' in response:
            with closing(response["AudioStream"]) as stream:
                self.output_interrupt = os.path.join(gettempdir(), 'speech2.mp3')

                with open(self.output_interrupt, 'wb') as file:
                    file.write(stream.read())
    
    def play_response_interrupt(self):
        self.is_speaking = True
        pygame.init()
        sound = pygame.mixer.Sound(self.output_interrupt)
